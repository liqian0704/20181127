...
public class TradeBizImpl extends SoaBaseBiz implements TradeBiz {
    ...
    @Override
    public ResponseMessage querySettle(SettleArgs args) {
        ResponseMessage resp = BeanValidator.validate(args);
        if (resp.isOk()) {
            // 是否需要汇总
            if (args.isCount()) {
                CountResponse count = customerSettleService.countByArgs(args);
                resp.setCount(count);
            }
            List<SettleDTO> settleDTOList = customerSettleService.queryByArgs(args);
            return resp.setContent(settleDTOList);
        }
        return resp;
    }
