...
import com.yeepay.g3.facade.opr.enumtype.PayModelTypeEnum;
...
public class OprYopOrderExtImpl extends SoaBaseBiz implements OprYopOrderExt {
    ...
    public YopCreateOrderResDTO createOrder(Order order, Customer customer, Payment payment) throws YmfTrxException {
        //@TODO 此处专为易码付台签流程定制 不要修改
        return createOrder(order, customer, payment, "1",null);
    }

    @Override
    public YopCreateOrderResDTO createOrder(Order order, Customer customer, Payment payment, String selfFlag,ScanQrCodeRequestDTO requestDTO)
        ...
        //产品信息
        buildGoodsInfo(customer, reqDto,order,requestDTO);
        //支付拓展参数 同人时传支付扩展参数,他人不用处理,此处会多一种方式,银联主扫需要加入支付扩展参数
        if(Constants.ONE_SELF_FLAG.equals(selfFlag)|| PaySource.OPEN_UPOP.equals(payment.getPaySource())){
            buildPaymentExtInfo(payment,customer, reqDto,requestDTO);
        }
        ...
        if(ResponseStatus.SUCCESS.equals(response.getStatus())){
            ...
            if(AppSourceEnum.LIKER.equals(appType)){
                reqDto.setMemo(AppType.AGENT.toString());//设置对账备注 后期考虑如何使用
            }else if(AppSourceEnum.ALLIANCE.equals(appType)){
                reqDto.setMemo(AppType.ALLIANCE.toString());//设置对账备注 后期考虑如何使用
            }
        }
        //行业拓展信息 可不填
     ...
     * 填充产品信息
     */
    private void buildGoodsInfo(Customer customer, YopCreateOrderReqDTO reqDto,Order order,ScanQrCodeRequestDTO requestDTO) {
        GoodsParamExtDTO goodsInfo = new GoodsParamExtDTO();
        goodsInfo.setGoodsDesc("商户单号：" + order.getCustomerOrderId());
        if(null != requestDTO){
            goodsInfo.setGoodsName(requestDTO.getProductName());
        }else{
            goodsInfo.setGoodsName(customer.getCustomerName()+"缴费");
        }
        //商品拓展信息 json类型
        reqDto.setGoodsParamExt(JSONUtils.toJsonString(goodsInfo));
    }

    private static final String defualtIp = "10.149.154.114";
    /**
     * 支付拓展参数
     */
    private void buildPaymentExtInfo(Payment payment, Customer customer, YopCreateOrderReqDTO reqDto,ScanQrCodeRequestDTO requestDTO) {
        //易码付 目前没有这些信息
        ...
        //此逻辑 只会在本人支付才会查询法人信息,银联主扫不需要查询法人信息
        if(!PaySource.OPEN_UPOP.equals(payment.getPaySource())){
...
//            paymentParamExtDto.setBankCardNo("");
            ...
            //银联主扫 新加入两个类型
            paymentParamExtDto.setPlatformType(payment.getPaySource().toString());
            //        微信,WECHAT;支付宝,ALIPAY；OPEN_UPOP,银联钱包
            paymentParamExtDto.setPayModelType(PayModelTypeEnum.DIRECT);
        }
        if (requestDTO != null) {
            if (ValidateParamUtil.isIp(requestDTO.getUserIp())) {
                paymentParamExtDto.setUserIp(requestDTO.getUserIp());
            } else {
                //默认值 10.149.154.114
                paymentParamExtDto.setUserIp(defualtIp);
            }
        }
