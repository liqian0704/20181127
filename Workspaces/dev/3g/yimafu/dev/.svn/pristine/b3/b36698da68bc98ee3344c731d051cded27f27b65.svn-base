...
public class CodBizImpl implements CodBiz {
	...
	public YmfOrderParam queryOrder(String customerNumber,String orderNo,String qrId) throws Exception {
		...
		log.info("[COD查单接口] 请求参数:{}" , JSONUtils.toJsonString(queryOrderParam));
		...
		log.info("[COD查单接口] 返回记录数:{} ,接口耗时:{}ms",orderResults.size(), (System.currentTimeMillis() - begin));
		if (orderResults.size() > 0) {
			log.info("[COD查单接口] COD返回数据:{}" , JSONUtils.toJsonString(orderResults.get(0)));
		}
	...
	public void orderPayNotify(Payment payment,Customer customer,Order order) {
		...
		if (null == codAsynInfo) {
			...
			log.info("创建通知记录成功,externalId :{},,商户单号:{}",order.getExternalId(),order.getCustomerOrderId());
		} else {
			if (Status.SUCCESS.equals(codAsynInfo.getStatus())) {
				log.info("已存在成功通知记录 ,externalId :{} ,商户单号:{}",order.getExternalId(), order.getCustomerOrderId());
			}
		...
		log.info("-----[COD支付通知接口] 通知COD报文:{}" ,JSONUtils.toJsonString(orderPayParam));
		...
		log.info("通知COD接口 耗时: {}ms ,系统流水号:{},商户订单号:{}",timecost ,order.getExternalId(),
				order.getCustomerOrderId());

		if (response) {
			log.info("通知COD成功,系统流水号:{},商户订单号:{}",order.getExternalId(),order.getCustomerOrderId());
			codAsynInfo.setStatus(Status.SUCCESS);
		} else {
			log.info("通知COD失败,系统流水号:{},商户订单号:{}",order.getExternalId(),order.getCustomerOrderId());
			codAsynInfo.setStatus(Status.FAIL);
		}
