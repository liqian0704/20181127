...
public class NCApiSupport {
     ...
     * 微信类交易的payType值
     */
	private static final int[] payCode = { 19, 20, 21, 22,30 };
     ...
     *           移动收银台绑卡支付(27) ,wap支付宝主扫支付(28),api支付宝主扫支付(29),API微信主扫支付(30) }
...
//		order.setFee("");
        ...
        String orderState = dto.getOrderState();//获取支付回调订单状态码
...
//		payment.setBankOrderId();//未传
        if (StringUtils.isNotBlank(dto.getTransactionId())) {
            payment.setOutOrderId(dto.getTransactionId());// 微信/支付宝订单号
        }
...
//	}
    ...
    /**
     *
     * 待支付  UNPAY(0)             INIT("未支付")
     * 支付成功 PAY_SUCCESS(1)      SUCCESS("订单完成")
     * 订单已撤消 CANCEL(2)          FAIL("订单失败")
     * 阻断交易 STOP(3)             FAIL("订单失败")
     * 支付失败 PAY_FAIL(4)         FAIL("订单失败")
     * 处理中 PROCESSING(5)        PROCESS("支付中")
     *
     * INIT("未支付"),
     PROCESS("支付中"),
     SUCCESS("订单完成"),
    ...
    /**
     * 根据订单业务类型判断下单是否使用商户订单号.
     * @param businessType  订单业务类型
     * @return
     */
    public static boolean useCustomerOrderId(BusinessType businessType) {
        if (businessType == null) {
            throw new RuntimeException("Param can not be null");
        }
        //有新增业务类型需在此处统一修改。
        if (businessType == BusinessType.ORDER_PAY ||
                businessType == BusinessType.REQUIRE_PAY) {
            return true;
        }
        return false;
    }
