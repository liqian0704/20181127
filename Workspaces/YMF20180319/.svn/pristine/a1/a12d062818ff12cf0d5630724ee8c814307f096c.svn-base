...
<div class="Container">
    <div class="Content fontText">
        <form action="" id="form" method="get">
            ...
            <div class="search">
                <div class="search_tit">
                    <h2 class="fw fleft f14">来客功能查询</h2>
                </div>
                ...
                <div class="search_con">
                    <ul id="list" class="fix">
                        <li>
                            <p>
                                ...
                                <input type="text" class="input_text" name="functionname" id="functionname"
                                       value="${role.roleCode}"/>
                            </p>
                        </li>
                        <li>
                            <p>
                                ...
                                <input type="text" class="input_text" name="functioncode" id="functioncode"
                                       value="${role.roleCode}"/>
                            </p>
                        </li>
                        <li>
                            <p>
                                <label class="text_tit">功能编码：</label>
                                <select class="select" name="functionlevel" id="functionlevel">
                                    <option value="">请选择</option>
                                    <c:forEach var="enumv" items="<%=FuncLevelEnum.values() %>" varStatus="status">
                                        <option value="<c:out value="${app:convert2FunQuery(enumv)}"/>">
                                            <c:out value="${enumv.bossDisplayName }"/>
                                        </option>
                                    </c:forEach>
                                </select>
                            </p>
                    ...
                    <div class="btn">
                        ...
                        <input type="button" class="btn_sure fw" onclick="addFunc()" value="新增功能"/>
                    </div>
                </div>
            </div>
        </form>
        <div style="margin: 0px auto;">
            <div>
                <q:table queryService="ymfQueryService" queryKey="appFunQuery" class="list" formId="form">
                    ...
                    <q:column title="操作" escapeHtml="false">
                        <a href="javascript:modifyFun('${id}')">修改</a>
                        <c:choose>
                            <c:when test="${available eq 1}">
                                <a href="javascript:toggleFun('${id}')">禁用</a>
                            </c:when>
                            <c:otherwise>
                                <a href="javascript:toggleFun('${id}')">启用</a>
                            </c:otherwise>
                        </c:choose>
                    </q:column>
                </q:table>
...
<script type="text/javascript">
    function toggleFun(id) {
        $.ajax({
            url: "${pageContext.request.contextPath}/user/permission/function/toggleFunc",
            type: "post",
            data: "id=" + id,
    ...
    function modifyFun(id) {
        window.location = "${pageContext.request.contextPath}/user/permission/function/role/toModify?id=" + id;
    }


    function addFunc() {
        window.location = "${pageContext.request.contextPath}/user/permission/function/toAdd";
    }
</script>
</body>
</html>
