...
public class ProfitSummationServiceImpl implements ProfitSummationService {
    ...
    @Override
    public List<ProfitSummation> findByMonth(String month,Status status) {
        if ( month == null) {
            throw new RuntimeException("month can't be null");
        }
        List<ProfitSummation> list = profitSummationDao.findByMonth(month,status);
        return list;
    }
    ...
    public Integer deleteByMonth(@Param("month") String month) {
        ...
        return profitSummationDao.deleteByMonth(month);
    }
