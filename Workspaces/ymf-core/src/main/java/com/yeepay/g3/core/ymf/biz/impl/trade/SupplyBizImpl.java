...
public class SupplyBizImpl extends SoaBaseBiz implements SupplyBiz {
    ...
    /**
     * 根据易码付业务订单号进行补单。
     * @param externalId   业务订单号
     * @return
     */
    @Override
    public ResponseMessage supplyOrderByExernalId(String externalId) {
        try {
            Order order = orderService.findByCustomerAndExternalId(null, externalId);
            if (order == null) {
                return ResponseMessage.error("订单不存在！");
            }
            Payment payment = paymentService.findByOrderIdAndPayStatusAndTrxType(order.getId(),
                    null,TrxType.PURCHASE);
            return supplyOrderAndNotify(payment, order);
    ...
    /**
     * 根据paymentID查询订单，并补发订单通知
     * @param paymentId     paymentId
     * @return
     * @throws YmfException 业务异常
     * @throws ParameterInvalidException    参数异常
     */
    @Override
     ...
     */
    private ResponseMessage supplyOrderAndNotify(Payment payment, Order order) throws YmfException,
        ...
        if (OrderStatus.SUCCESS.equals(orderStatus)) {
            notifyCodOrLiker(payment, customer, order);
        ...
        } else if (OrderStatus.INIT.equals(orderStatus)) {
            return ResponseMessage.error("该订单暂未支付！");
        }
        //根据订单下单方进行查单。
        if (TradeSystemEnum.OPR == order.getTradeSystem()) {
            //大算下单
            ...
            YopQueryOrderResDTO orderResDTO = null;
            try {
                orderResDTO = oprYopOrderExt.queryOrder(
                        order.getCustomerNumber(), orderId, payment.getYeepayOrderId());
            } catch (YmfTrxException e) {
                return ResponseMessage.error("查单接口调用失败，请稍后重试！");
            }
            ...
            if ("SUCCESS".equals(status)) {
                ...
                payment.setCardType(CardType.getCardType(orderResDTO.getCardType().toString()));
                ...
                return ResponseMessage.error("订单未支付，请稍后再试！");
            }
        } else {
            //收银台下单
    ...
    /**
     * 根据商户业务类型判断通知like系统或cod系统
     */
    private void notifyCodOrLiker(Payment payment, Customer customer, Order order) {
        Business business = businessService.getBusinessById(customer.getBusinessId());
        if(Constants.LAIKE.equals(business.getBizCode())){
            likerBizService.doNotify(payment,customer, order);
            log.info("------ Liker Notify Success");
        } else {
            // 发起COD通知
