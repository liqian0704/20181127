...
function doQuery(){
    ...
    var customerNumber = $("#customerNo").val().trim();
    ...
    if (customerNumber == null || customerNumber == "") {
		Tools.alert("参数错误，未获取商编");
    	return false ;
	}
    var requestParm = "customerRequestId=" + orderNum +"&customerNumber=" + customerNumber +
			"&qrCode=" + qrCode;
    $.ajax({
        url : "${pageContext.request.contextPath }/orderPay/queryCodOrder",
        type: "POST",
        data: requestParm,
        dataType: "json",
...
function parseToPayPage(resp,order){
    ...
    $("#form2").attr('action','${pageContext.request.contextPath }/orderPay/orderDetail');
    $("#form2").submit() ;
}
</script>
</head>
...
<br/>
	...
	<div class="plate">
		<span></span>
		<input class="input-text" name="orderNo" id="orderNo"  type="text" autofocus="autofocus" placeholder="缴费通知单号 | 身份证 | 车牌号">
	</div>
	<div class="btn-box">
