...
import com.yeepay.g3.core.ymf.utils.*;
...
import com.yeepay.g3.utils.query.jdbc.*;
 ...
 */
public class YmfDownServiceImpl extends QueryServiceImpl implements YmfDownService {
    ...
    public QueryResult query(String queryKey, QueryParam queryParam) {
        ...
        Query queryConfig = this.findQuery(queryKey);
        if(queryConfig == null) {
            throw new QueryException("no query defined with key : " + queryKey);
        } else {
            DataSource ds = this.findDataSource(queryConfig.getDataSource());
            QueryRequest queryRequest = this.createQueryRequest(queryConfig, queryParam.getParams());
            if(queryRequest == null) {
                ...
                if("isDown".equals(queryParam.getParams().get("isDown"))){//下载接口走
                    String downFileName=null;
                    try {
                        downFileName= dataDown(queryParam, ds, sql, queryRequest);
                        logger.info("文件生成成功");
                    }catch (Exception e){
    ...
    private String dataExcelDown(String filePath,QueryParam queryParam,
                               IJdbcQueryer queryer,int page,String sql,QueryRequest queryRequest,  String[] filedNames, String[] fileds, List<Format>  formatList){
            String msg=null;
            File file = new File(filePath);
            SimpleExcelWriter excel = null;
            String fileName = String.valueOf(queryParam.getParams().get("ymfOperatorSe"))+"-"+Calendar.getInstance().getTimeInMillis()+ ".xlsx";
    ...
    private String dataTxtDown(String filePath,QueryParam queryParam,
        ...
        String fileName=String.valueOf(queryParam.getParams().get("ymfOperatorSe"))+"-"+Calendar.getInstance().getTimeInMillis()+ ".txt";
        String titleSplit="   ";
        String readStr="|";
    ...
    //下载接口
    private String dataDown(QueryParam queryParam,DataSource ds,String sql,QueryRequest queryRequest) throws Exception{
        IJdbcQueryer queryer = JdbcQueryerFactory.createJdbcQueryer(queryParam.isForDal(), ds);
        String msg=null;
        //获得总条数
        int totalcount=this.count(ds,sql,queryRequest.getSqlParams());
//        String filePath=File.separator+"Users"+File.separator+"yp-tc-m-2803"+File.separator+"work"+File.separator+"testexcel2"+File.separator;
       String filePath = FileManageUtils.getFilePath();
        logger.info(filePath);
        //文件下载获取样式
        String filename=queryParam.getParams().get("downXml")+".xml";
        ...
        //excel操作
        if("downExcel".equals(queryParam.getParams().get("downtype"))){
            msg= dataExcelDown(filePath,queryParam,queryer,page,sql,queryRequest,filedNames,fileds,formatList);

        }
        //txt操作
        if("downTxt".equals(queryParam.getParams().get("downtype"))){
            msg= dataTxtDown(filePath,queryParam,queryer,page,sql,queryRequest,filedNames,fileds,formatList,totalcount);
    ...
    private Query findQuery(String key) {
        if(this.querys == null) {
            throw new QueryException("querys is empty !");
        } else {
            Query query = (Query)this.querys.get(key);
            if(query == null) {
