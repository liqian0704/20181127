...
//}
    ...
    public void testTotalBillProcessByBillNoRecord() throws IOException, ParseException {
        ...
        //算出账单周期开始时间
        String billStartDate = BusinessUtil.getCDate("1042781335", "PROMOTER_BILL");
...
//        }
        ...
        if (startDate == null) {
            ...
            System.out.println("输出账单截至时间" + BusinessUtil.getNDate("1042781335", "PROMOTER_BILL"));
            billNoEndDate = sdf.parse(BusinessUtil.getNDate("1042781335", "PROMOTER_BILL"));
            if (date.getTime() < billNoEndDate.getTime()) {
                ...
                BillStatusEnum billStatus=BusinessUtil.selectTotalBillByType("1042781335",billStartDate,"PROMOTER_BILL").get(1).getBillStatus();

                Assert.assertTrue(billStatus.equals(BillStatusEnum.INIT));}

                else {
                    System.out.println("本期汇总账单已经生成,无需再次生成");
                }
