...
public class BeanValidator {
    ...
    public static ResponseMessage validate(Object o) {
        ...
        Map<Field, List<Annotation>> classValidators = fetchClassValidators(o.getClass());
        if (null != classValidators) { // 如果实体注册了validator的注解
            try {
                for (Iterator<Map.Entry<Field, List<Annotation>>> it = classValidators.entrySet().iterator(); it.hasNext(); ) {
    ...
    public static ResponseMessage validateInclude(Object o, String...args) {
        ...
        Map<String, List<Annotation>> classValidators = fetchNameClassValidators(o.getClass());
        if (null != classValidators) { // 如果实体注册了validator的注解
            try {
                for (String arg : args) {
                    List<Annotation> annotations = classValidators.get(arg); // 从缓存中取出的标有注解的
                    if (null != annotations) {
                        try { // 反射获取Field
                            Field field = ReflectionUtils.getField(o.getClass(), arg);
                            field.setAccessible(true);
                            Object value = field.get(o);
    ...
    public static ResponseMessage validateExclude(Object o, String...args) {
        ...
        if (null != classValidators) { // 如果实体注册了validator的注解
            try {
                for (Iterator<Map.Entry<String, List<Annotation>>> it = classValidators.entrySet().iterator(); it.hasNext(); ) {
                    ...
                    if (!ArrayUtils.contains(args, fieldName)) { // 没找到才校验
                        ...
                        if (null != annotations) {
                            // 反射获取Field
                            Field field = ReflectionUtils.getField(o.getClass(), fieldName);
                            field.setAccessible(true);
                            Object value = field.get(o);
                            for (Annotation annotation : annotations) {
                                ResponseMessage resp = validate(value, annotation);
     ...
     * @return
     */
    private static Map<Field, List<Annotation>> checkInitialized(Class clazz) {
     ...
     * @return
     */
    private static Map<String, List<Annotation>> checkNameInitialized(Class clazz) {
        log.info("初始化校验类缓存, clazz: " + clazz.getCanonicalName());
