...
public class YMFTradeBizFacadeImpl extends SoaBaseBiz implements YMFTradeBizFacade {
    ...
    public ScanQrCodeResponseDTO scanQrCodeDoPay(ScanQrCodeRequestDTO requestDto) throws Exception {
        ...
        // 秒掉风险校验
        if (requestDto.getBalanceType() == BalanceType.S0
                && (requestDto.getPaySource() == PaySource.ALIPAY
                || requestDto.getPaySource() == PaySource.WECHAT
                || requestDto.getPaySource() == PaySource.SCAN_CODE)) {
            //校验S0订单笔数限制
            boolean b = orderCheckBiz.checkCustomerS0Limit(requestDto.getCustomerNumber());
            if (!b) {
                logger.warn("当前商户秒到交易存在风险");
                response.setReturnCode(TrxCode.T1041.getCode());
                //response.setReturnMsg(TrxCode.T1041.getMsg());
                response.setReturnMsg("交易存在风险，请选择其他收款方式");
                return response;
            }
        }
...
//                }
    ...
    private Payment initPassivePayment(PassivePayRequestDTO requestDto) {
        ...
        payment.setPaySource(requestDto.getPaySource());
        payment.setExpireTime(ExpireTimeUtil.expTime(new Date(),BusinessType.STANDARD,null));
        return payment;
    }
