...
<form id="form">
    <div class="information">
        ...
        <div class="input_cont">
            <ul >
                <li>
                    <p>
                        ...
                        <input type="text" class="input_text" id="funName"  name="funName" value="${funcRole.functionName}"/>
                    </p>
                </li>
            </ul>
            <ul >
                <li>
                    <p>
                        ...
                        <input type="text" class="input_text" id="funCode"  name="funCode" value="${funcRole.functionCode}"/>
                    </p>
                </li>
            </ul>
            ...
            <input type="hidden" name="id" value="${funcRole.id}"/>
            <div class="btn">
                <input type="button"  class="btn_sure fw" id="queryid" onclick="modifyFunc();" value="修改" />
...
<script>
    ...
    function modifyFunc() {
        if (is_checked()) {
            ...
            var options = {
                type: "post",
                url: "${pageContext.request.contextPath}/user/permission/function/role/modify",
                data: data,
                dateType: "json",
    ...
    var funcLevelList = ${funcLevelList};
    ...
    function initNodes(){
        ...
        for (var i=0; i < nodes.length; i++) {
            for(var j=0; j < roleIds.length; j++){
                if(nodes[i].id == roleIds[j]){
                    treeObj.checkNode(nodes[i], true, true);
                }
            }
        }
        for(var i=0; i < funcLevelList.length; i++){
            for(var j=0; j < funcLevel.length; j++){
                if(funcLevelList[i]== funcLevel[j].value ){
                    funcLevel[i].checked = true;
                }
            }
        }
    }
