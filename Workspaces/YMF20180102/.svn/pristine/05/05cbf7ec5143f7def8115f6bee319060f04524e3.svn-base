...
public abstract class AbstractProfitCalculator implements ProfitCalculator {
     ...
     * 蜜糖增量商户，支付给易宝0.1%
     */
    protected static BigDecimal MT_TRX_COST_RATE = new BigDecimal(0.0001);
    /**
     * 蜜糖增量商户,每笔结算收取的费用
     */
    protected static BigDecimal MT_SETTLE_COST = new BigDecimal(0.1);
    /**
     * （后决定按照天数计算）蜜糖增量商户,T1每个商户大约的结算笔数
     */
    //protected static BigDecimal MT_SETTLE_COUNT = new BigDecimal(5);
    ...
    protected ProfitSummation querySummation(String month,boolean reCaculate) {
        ...
        if (CustomerTypeEnum.STOCK == getCustomerType()) {
            ...
            BigDecimal mtTotalProfit = calculateMtProfit(totalTrxAmt, totalProfitAmt,
                    summation.getTotalCount(),getPercent(),summation.getTotalDayCount());
    ...
    protected void calculateStockOne(ProfitSummation summation, Profit profit,BigDecimal percent) {
        ...
        if (trxAmt == null) {
            trxAmt = new BigDecimal(0);
        }
        if (profitAmt == null) {
            profitAmt = new BigDecimal(0);
        }
    ...
    protected void calculateMtOne(ProfitSummation summation, Profit profit, BigDecimal trxRate) {
        ...
        BigDecimal mtProfit = calculateMtProfit(trxAmt, profitAmt, 1L,trxRate,profit.getDayCount());
        //单笔蜜糖毛利金额
    ...
    protected BigDecimal calculateMtProfit(BigDecimal totalTrxAmt, BigDecimal totalProfitAmt,
        ...
        BigDecimal dayCount = new BigDecimal(dayCouont);
        // 结算成本
        BigDecimal settleCost = dayCount.multiply(new BigDecimal(customerCount));
