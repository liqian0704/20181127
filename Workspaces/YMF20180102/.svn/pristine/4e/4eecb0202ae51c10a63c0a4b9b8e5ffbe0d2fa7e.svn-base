...
public class SkbStockCalculatorImpl extends AbstractProfitCalculator {
     ...
     * 蜜糖低于9.6分配占比
     */
    private static final BigDecimal MT_BELOWPART_RATE = new BigDecimal(0.2);
    /**
     * 蜜糖高于9.6占比
     */
    private static final BigDecimal MT_ABOVEPART_RATE = new BigDecimal(0.5);
    /**
     * 产品类型
     */
    ...
    private BigDecimal calculateStockSkbProfitPercent(ProfitSummation summation){
        ...
        //4.计算占比,平均比例
        //蜜糖存量商户最终分润金额
        BigDecimal mitangTotalProfitAmt = new BigDecimal(0);
        BigDecimal mitangTotalTrxAmt = new BigDecimal(0);
        //蜜糖低于9.6的分润金额
        BigDecimal mittangBelowPartProfitAmt = new BigDecimal(0);
        //蜜糖高于9.6的分润金额
        BigDecimal mittangAbovePartProfitAmt = new BigDecimal(0);

        BigDecimal belowPartTrxAmt = new BigDecimal(0);
        BigDecimal abovePartTrxAmt = new BigDecimal(0);
        //零的校验
        ...
        BigDecimal belowPartPercent = belowPartTrxAmt.divide(skbStockTotalTrxAmt,2, BigDecimal.ROUND_HALF_UP);
        mittangBelowPartProfitAmt = skbStockTotalProfitAmt.multiply(belowPartPercent).multiply(MT_BELOWPART_RATE);

        BigDecimal abovePartPercent = abovePartTrxAmt.divide(skbStockTotalTrxAmt,2, BigDecimal.ROUND_HALF_UP);
        ...
        //计算蜜糖平均分配比例
        BigDecimal mitangAveragePercent = mitangTotalProfitAmt.divide(skbStockTotalProfitAmt,2, BigDecimal.ROUND_HALF_UP);
